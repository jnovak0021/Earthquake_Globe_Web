...
const UserInterface: React.FC<UserInterfaceProps> = ({ backendName }) => {
   ...
   useEffect(() => {
      const fetchData = async () => {
         try {
            ...
            setUsers(response.data.reverse()); // Reverse the order to show the latest user first
         } catch (error) {
   ...
   return (
      <div className={`user-interface ${bgColor} ${backendName} w-full max-w-md p-4 my-4 rounded shadow`}>
         {/* <img ={`/${backendName}logo.svg`} alt={`${backendName} Logo`} className="w-20 h-20 mb-6 mx-auto" /> */}
