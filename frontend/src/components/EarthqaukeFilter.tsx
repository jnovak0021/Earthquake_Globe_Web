...
const EarthQuakeFilter: React.FC = () => {
    ...
    const fetchPageData = async (offset: number, limit: number): Promise<Earthquake[]> => {
        const response = await fetch(`https://earthquake.usgs.gov/fdsnws/event/1/query?format=geojson&starttime=${startTime}&endtime=${endTime}&minmagnitude=${minMagnitude}&maxmagnitude=${maxMagnitude}&mindepth=${minDepth}&maxdepth=${maxDepth}&offset=${offset}&limit=${limit}`);
    ...
    const fetchAllData = async (): Promise<Earthquake[]> => {
        ...
        while (hasMoreData) {
            const promises = [];
            for (let i = 0; i < 100; i++) { // Adjust the number of concurrent requests as needed
    ...
    return (
        <div className="bg-gray-200 p-4 rounded-lg shadow-md">
            ...
            <div className="mb-2">
                ...
                <input
                    type="text"
                    placeholder="KML Color By"
                    value={kmlColorBy}
                    onChange={(e) => setKmlColorBy(e.target.value)}
                    className="w-full p-2 mb-2 border border-gray-300 rounded text-black"
                />
            </div>
            <button
